...
def user_summary():
    ...
    for subject in subjects:
        ...
        attempts = Score.query.filter_by(user_id=current_user.id).all()
        ...
        for quiz in quizzes:
            if quiz.id in attempted_quizzes:
                score = Score.query.filter_by(user_id=current_user.id, quiz_id=quiz.id)
                quiz = Quiz.query.get(quiz.id)
                percent_score = score.score / quiz.maximum_marks * 100 if quiz.maximum_marks > 0 else 0
    ...
    return render_template('user_summary.html', user=current_user, subjects=subjects, chart_data=chart_data)


@app.route('/user_summary/subject/<int:subject_id>')
@login_required
def user_subject_summary(subject_id):
    ...
    chart_data = []
    for quiz in quizzes:
        attempt = Score.query.filter_by(user_id=current_user.id, quiz_id=quiz.id).first()
    ...
    return render_template('user_subject_summary.html', subject=subject, quizzes=quizzes, chart_data=chart_data)

@app.route('/user_summary/subject/<int:subject_id>/chapter/<int:chapter_id>')
@login_required
def user_chapter_summary(subject_id, chapter_id):
    subject = Subject.query.get_or_404(subject_id)
    chapter = Chapter.query.get_or_404(chapter_id)
    quizzes = chapter.quizzes
    chart_data = []
    for quiz in quizzes:
        user_attempt = Score.query.filter_by(user_id=current_user.id, quiz_id=quiz.id).first()
